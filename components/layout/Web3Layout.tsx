...
export default function Layout({ children, paddingTop, hideDesktopMenu, alwaysVisible }: PropsWithChildren<LayoutProps>) {
  const isMounted = useIsMounted()
  const { isSignedIn, isSupportedChain } = useAuth();
  const router = useRouter();
  
  const mainContainer = <div style={{paddingTop: paddingTop ? paddingTop + 'px' : 0}}>
    {children}
  </div>

  return (<>
    <nav className={`navbar fixed-top navbar-expand-lg py-3 ${roboto.className}`}>
